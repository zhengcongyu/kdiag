...
- client-go SharedInformer 增量监听 9 类 Kubernetes 资源，处理缓存同步、
...
- PostgreSQL 迁移与 Repository、异步 REST API、完整命名 SSE 事件、结构化日志、
  请求 ID、指标、超时与优雅关闭。
- React + TypeScript 控制台：概览、Incident 列表/详情、证据、拓扑、时间线、
  资源诊断、网络诊断和历史回放。
...
- Collector、拓扑、规则、Repository 与 API 已分别实现并测试，但尚未连成一个实时
  集群 Controller 进程。
